...
top: -20px"
          >
            ...
            <div class="cc elevation-9" style="border-radius:20px;">
              <v-toolbar class="blue-grey darken-3">
                <h1 class="dislpay-1" style="text-align:left; padding:10px; color:white">Portofolio</h1>
                <v-spacer></v-spacer>
                <v-toolbar-title>
                  <v-text-field
                    solo
                ...
                <v-btn color="blue-grey darken-2" dark class="mb-2" @click="dialog = true">
                  <v-icon left dark>add</v-icon>Portofolio
                </v-btn>
              </v-toolbar>
            </div>
          </div>
        ...
        >
          <template v-slot:items="props">
            <td>
              <img :src="getUrl+'/'+props.item.foto" alt="" style="width : auto; max-height : 250px;">
            </td>
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-card-title>
          <span class="headline">Tambah Porto </span>
        </v-card-title>
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12>
                <v-text-field v-model="createitem.judul" label="judul Porto" :rules="notNull"></v-text-field>
              </v-flex>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              ...
              <v-flex xs12>
                <v-text-field v-model="editedItem.judul" label="judul porto" :rules="notNull"></v-text-field>
              </v-flex>
...
editedIndex : -1 ,
  ...
  methods: {
    ...
    editItem(item) {
      this.edit = true;
      this.editedIndex = this.portos.indexOf(item);
    ...
    async deleteItem(items) {
      const index = this.portos.indexOf(items);
      ...
      if (swal.value) {
        let r = await axios.delete("/admin/porto/" + item._id,{
            headers: {
                Authorization: `bearer ${this.token}`
            }
    ...
    async save() {
      ...
      let ress = await axios.post("/admin/porto", fd,{
            headers: {
                Authorization: `bearer ${this.token}`
            }
   ...
   async edited() {
      ...
      let ress = await axios.put("/admin/porto/"+this.editedItem._id, fd,{
            headers: {
                Authorization: `bearer ${this.token}`
            }
    ...
    headline(items,hl) {
     const index = this.portos.indexOf(items);
      let item = Object.assign({}, items);
      axios.put(`/admin/headline/${item._id}`, {headline : hl},{
            headers: {
                Authorization: `bearer ${this.token}`
            }
