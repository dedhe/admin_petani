          ...
          >
            ...
            <div class="cc elevation-9" style="border-radius:20px;">
              <v-toolbar class="blue-grey darken-3">
                <h1 class="dislpay-1" style="text-align:left; padding:10px; color:white">Layanan</h1>
                <v-spacer></v-spacer>
                <v-toolbar-title>
                  <v-text-field
                    solo
                ...
                <v-btn color="blue-grey darken-2" dark class="mb-2" @click="dialog = true">
                  <v-icon left dark>add</v-icon>Layanan
                </v-btn>
              </v-toolbar>
            </div>
          </div>
...
computed: {
  ...
  methods: {
    ...
    async deleteItem(items) {
      const index = this.layanan.indexOf(items);
      ...
      if (swal.value) {
        let r = await axios.delete("/admin/layanan/" + item._id,{
            headers: {
                Authorization: `bearer ${this.token}`
            }
    ...
    async save() {
      ...
      let ress = await axios.post("/admin/layanan", data,{
            headers: {
                Authorization: `bearer ${this.token}`
            }
